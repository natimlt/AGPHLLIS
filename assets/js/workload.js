...
class WorkloadManager {
     ...
     * selected period, so the actual unit of work (a test performed) is
     * what gets entered - not a single number per department. The
     * department summary below is auto-computed by rolling test counts
     * up to their department, then "Apply to Report" sums those into
     * the category fields above (workload itself is one aggregate
     * record per month, not one per department/test).
     */
    renderBulkTable() {
        ...
        if (catalog.length === 0) {
            tbody.innerHTML = `<tr><td colspan="3" class="text-center" style="padding:1.5rem;">No tests in the catalog yet</td></tr>`;
            this.renderDeptSummary();
            return;
        }

        // Group tests by department so the entry table reads naturally
        // instead of one long undifferentiated list.
        const byDept = {};
        catalog.forEach(t => {
            const dept = t.department || 'Unspecified';
        ...
        Object.keys(byDept).sort().forEach(dept => {
            html += `<tr style="background:var(--gray-50);"><td colspan="3" style="font-weight:600; padding:0.5rem 0.75rem;">${Utils.escapeHtml(dept)}</td></tr>`;
            byDept[dept].forEach(t => {
                html += `
                <tr data-test="${Utils.escapeHtml(t.testName)}" data-dept="${Utils.escapeHtml(dept)}">
        ...
        // Live-update the department summary as counts are typed in.
        tbody.querySelectorAll('.bulk-tests').forEach(input => {
            input.addEventListener('input', () => this.renderDeptSummary());
        });
        this.renderDeptSummary();
    }

    /** Auto-computed rollup: sums the per-test bulk entries up to their department. */
    renderDeptSummary() {
        const container = document.getElementById('workloadDeptSummaryBody');
        if (!container) return;

        const rows = document.querySelectorAll('#workloadBulkTableBody tr[data-test]');
        const byDept = {};
        rows.forEach(row => {
            const dept = row.dataset.dept;
            const count = parseInt(row.querySelector('.bulk-tests').value) || 0;
            byDept[dept] = (byDept[dept] || 0) + count;
        });

        const depts = Object.keys(byDept).sort();
        if (depts.length === 0 || depts.every(d => byDept[d] === 0)) {
            container.innerHTML = `<tr><td colspan="3" class="text-center" style="padding:1rem; color:var(--gray-400);">Enter test counts above to see the department summary</td></tr>`;
            return;
        }

        const total = depts.reduce((sum, d) => sum + byDept[d], 0);
        container.innerHTML = depts.map(d => `
            <tr>
                <td>${Utils.escapeHtml(d)}</td>
                <td>${byDept[d]}</td>
                <td>${total ? Math.round((byDept[d] / total) * 100) : 0}%</td>
            </tr>`).join('') +
            `<tr style="font-weight:700; border-top:2px solid var(--gray-300);"><td>Total</td><td>${total}</td><td>100%</td></tr>`;
    }
